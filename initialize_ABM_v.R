...
LOS <- read.csv('reslos_freq_2020.csv')
...
LOS %>%
  ...
  rename(LOS = F_reslos2)-> LOS # setting a minimum of a week stay - may need to adjust
...
initialize <- function(inits,params,dt){
  ...
  if (S.rNC.init > 0){
    ...
    S.rNC=as.data.frame(cbind("ID"=as.numeric(),"VL"=as.numeric(), 
                              ...
                              "V_doses" = as.numeric(),"V_type" = as.numeric(), "V_refused" = as.numeric(),  
  ...
  if (E.rNC.init > 0){
    E.rNC=as.data.frame(cbind("ID"=(total+1):(total + E.rNC.init),"VL"=rep(0,E.rNC.init), 
                              ...
                              "V_doses" = rep(0,E.rNC.init),"V_type" = rep(NA,E.rNC.init), "V_refused" = rep(NA,E.rNC.init),  
  ...
  if (A.rNC.init > 0){
    A.rNC=as.data.frame(cbind("ID"=(total+1):(total+ A.rNC.init), "VL_rise" = round(runif(A.rNC.init, 1, 4)),
                              ...
                              "V_doses" = rep(0,A.rNC.init),"V_type" = rep(NA,A.rNC.init), "V_refused" = rep(NA,A.rNC.init),  
  ...
  if (I.rNC.init > 0){
    I.rNC=as.data.frame(cbind("ID"=(total+1):(total+ I.rNC.init), "VL_rise" = round(runif(I.rNC.init, 1, 4)),
                              ...
                              "V_doses" = rep(0,I.rNC.init),"V_type" = rep(NA,I.rNC.init), "V_refused" = rep(NA,I.rNC.init),  
  ...
  if (R.rNC.init > 0){
    R.rNC=as.data.frame(cbind("ID"=(total+1):(total + R.rNC.init),"VL"=rep(NA,R.rNC.init), "VL_waning"=rep(NA,R.rNC.init),
                              ...
                              "V_doses" = rep(0,R.rNC.init),"V_type" = rep(NA,R.rNC.init), "V_refused" = rep(NA,R.rNC.init),  
  ...
  if (I.rC.init > 0){
    I.rC=as.data.frame(cbind("ID"=(total+1):(total+ I.rC.init),"VL_rise" = round(runif(I.rC.init, 1, 4)),
                             ...
                             "V_doses" = rep(0,I.rC.init),"V_type" = rep(NA,I.rC.init), "V_refused" = rep(NA,I.rC.init),  
  ...
  if (R.rC.init > 0){
    R.rC=as.data.frame(cbind("ID"=(total+1):(total + R.rC.init),"VL"=rep(0,R.rC.init), "VL_waning"=rep(0,R.rC.init), 
                             ...
                             "V_doses" = rep(0,R.rC.init),"V_type" = rep(NA,R.rC.init), "V_refused" = rep(NA,R.rC.init),  
  ...
  if (S.hcwNC.init > 0){
    S.hcwNC=as.data.frame(cbind("ID"=(total+1):(total + S.hcwNC.init),"VL"=rep(NA,S.hcwNC.init), 
                                "V_doses" = rep(0,S.hcwNC.init),"V_type" = rep(NA,S.hcwNC.init), "V_refused" = rep(NA,S.hcwNC.init),  
  ...
  if (E.hcwNC.init > 0){
    E.hcwNC=as.data.frame(cbind("ID"=(total+1):(total + E.hcwNC.init),"VL"=rep(0,E.hcwNC.init),
                                "V_doses" = rep(0,E.hcwNC.init), "V_doses" = rep(NA,E.hcwNC.init), "V_refused" = rep(NA,E.hcwNC.init),  
  ...
  if (A.hcwNC.init > 0){
    A.hcwNC=as.data.frame(cbind("ID"=(total+1):(total+ A.hcwNC.init),"VL_rise" = round(runif(A.hcwNC.init, 1, 4)),
                                ...
                                "V_doses" = rep(0,A.hcwNC.init), "V_type" = rep(0,A.hcwNC.init), "V_refused" = rep(NA,A.hcwNC.init),  
  ...
  if (I.hcwNC.init > 0){
    I.hcwNC=as.data.frame(cbind("ID"=(total+1):(total+ I.hcwNC.init),"VL_rise" = round(runif(I.hcwNC.init, 1, 4)),
                                ...
                                "V_doses" = rep(0,I.hcwNC.init), "V_type" = rep(NA,I.hcwNC.init), "V_refused" = rep(NA,I.hcwNC.init),  
  ...
  if (R.hcwNC.init > 0){
    R.hcwNC=as.data.frame(cbind("ID"=(total+1):(total + R.hcwNC.init),"VL"=rep(0,R.hcwNC.init), "VL_waning"=rep(0,R.hcwNC.init),
                                "V_doses" = rep(0,R.hcwNC.init),"V_type" = rep(NA,R.hcwNC.init), "V_refused" = rep(NA,R.hcwNC.init),  
  ...
  if (S.hcwC.init > 0){
    S.hcwC=as.data.frame(cbind("ID"=(total+1):(total + S.hcwC.init),"VL"=rep(NA,S.hcwC.init), 
                               "V_doses" = rep(0,S.hcwC.init), "V_type" = rep(NA,S.hcwC.init), "V_refused" = rep(NA,S.hcwC.init),  
  ...
  if (E.hcwC.init > 0){
    E.hcwC=as.data.frame(cbind("ID"=(total+1):(total + E.hcwC.init),"VL"=rep(0,E.hcwC.init), 
                               "V_doses" = rep(0,E.hcwC.init),"V_type" = rep(NA,E.hcwC.init), "V_refused" = rep(NA,E.hcwC.init),  
  ...
  if (A.hcwC.init > 0){
    A.hcwC=as.data.frame(cbind("ID"=(total+1):(total+ A.hcwC.init), "VL_rise" = round(runif(A.hcwC.init, 1, 4)),
                               ...
                               "V_doses" = rep(0,A.hcwC.init), "V_type" = rep(0,A.hcwC.init), "V_refused" = rep(NA,A.hcwC.init),  
  ...
  if (I.hcwC.init > 0){
    I.hcwC=as.data.frame(cbind("ID"=(total+1):(total+ I.hcwC.init), "VL_rise" = round(runif(I.hcwC.init, 1, 4)),
                               ...
                               "V_doses" = rep(0,I.hcwC.init), "V_type" = rep(NA,I.hcwC.init), "V_refused" = rep(NA,I.hcwC.init),  
  ...
  if (R.hcwC.init > 0){
    R.hcwC=as.data.frame(cbind("ID"=(total+1):(total + R.hcwC.init),"VL"=rep(0,R.hcwC.init),"VL_waning"=rep(0,R.hcwC.init),
                               "V_doses" = rep(0,R.hcwC.init),"V_type" = rep(NA,R.hcwC.init), "V_refused" = rep(NA,R.hcwC.init),  
  ...
  if (I.hcwH.init > 0){
    I.hcwH=as.data.frame(cbind("ID"=(total+1):(total+ I.hcwH.init), "VL_rise" = round(runif(I.hcwH.init, 1, 4)),
                               ...
                               "V_doses" = rep(0,I.hcwH.init), "V_doses" = rep(NA,I.hcwH.init), "V_refused" = rep(NA,I.hcwH.init),  
  ...
  Ns <- list("S.rNC"=S.rNC,
             ...
             "vax_count1"=0,
             "vax_count2"=0, 
             "asymptomatic_inc_r_0"=0,
             "asymptomatic_inc_r_1"=0,
             "asymptomatic_inc_r_2"=0,
             "symptomatic_inc_r_0"=0,
             "symptomatic_inc_r_1"=0,
             "symptomatic_inc_r_2"=0,
             "asymptomatic_inc_hcw_0"=0,
             "asymptomatic_inc_hcw_1"=0,
             "asymptomatic_inc_hcw_2"=0,
             "symptomatic_inc_hcw_0"=0,
             "symptomatic_inc_hcw_1"=0,
             "symptomatic_inc_hcw_2"=0,
             "daily_visitors"=0,
             "res_coverage1"=0,
             "res_coverage2"=0, 
             "t_last_case"=NA)
