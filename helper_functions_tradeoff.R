recover_or_test_r <- function(df,parms,symptoms, testing_parameters){
  
  df %>%
    subset(Inf.days==Inf.pd) %>%
    mutate(Rec.days = 0) %>%
    dplyr::select(ID,VL,arrival,departure,VL_waning,Rec.days,
                  V_doses,V_type,V_refused,V1_t,V2_t,VE_i,VE_s,VE_p) -> recovered
  ...
  if(symptoms=="A"){    ## asymptomatics who are identified through testing
    df %>%
      subset(ID.pd==ID.days & VL>=testing_parameters[["VL.threshold.r"]] & is.na(removal.pd)) %>%
      mutate(removal.pd = ID.pd + testing_parameters[["test_delay_r"]]
             #,ID.days = ID.days + 1
             ) -> df1
    ...
    df %>%
      subset(ID.days == parms[["id.I"]] | (ID.days == ID.pd & VL>=testing_parameters[["VL.threshold.r"]] & is.na(removal.pd))) %>%
      mutate(removal.pd = case_when(ID.days == parms[["id.I"]] ~ ID.days, 
                                   (ID.days == ID.pd & VL>=testing_parameters[["VL.threshold.r"]]) ~ ID.pd + testing_parameters[["test_delay_r"]]) #ID.days != parms[["id.I"]] &
             # ,ID.days = ID.days + 1
  ...
  df %>%
    subset(ID.pd==ID.days & VL<testing_parameters[["VL.threshold.r"]] & is.na(removal.pd)  & !(ID %in% df1$ID)) %>%  # !(ID %in% df1$ID) excludes people identified through symptoms
    mutate(Inf.days=Inf.days + 1,
           ID.pd = ID.pd + testing_parameters[["test_freq_r"]],
           ID.days = ID.days + 1,
...
recover_or_test_hcw <- function(df,parms, total, symptoms, ratio, testing_parameters){
  ...
  if(symptoms=="A"){    ## asymptomatics who are identified through testing
    df %>%
      subset(ID.pd==ID.days & VL>=testing_parameters[["VL.threshold.hcw"]] & is.na(removal.pd)) %>%
      mutate(removal.pd = ID.pd + testing_parameters[["test_delay_hcw"]])  -> df1
    ...
    df %>%
      subset(ID.days == parms[["id.I"]] | (ID.days == ID.pd & VL>=testing_parameters[["VL.threshold.hcw"]] & is.na(removal.pd))) %>%
      mutate(removal.pd = case_when(ID.days == parms[["id.I"]] ~ ID.days, 
                                    (ID.days == ID.pd & VL>=testing_parameters[["VL.threshold.hcw"]]) ~ ID.pd + testing_parameters[["test_delay_hcw"]])
             ) -> df1  ##  ## symptomatics who are identified through testing OR symptoms
  }
  
  
  df %>%
    subset(ID.pd==ID.days & VL<testing_parameters[["VL.threshold.hcw"]] & is.na(removal.pd) & !(ID %in% df1$ID)) %>%
    mutate(Inf.days=Inf.days + 1,
           ID.days=ID.days + 1,
           ID.pd = ID.pd +  testing_parameters[["test_freq_hcw"]],
           VL = case_when(Inf.days <=VL_rise ~ VL * (Inf.days+1)/(Inf.days),
                          Inf.days > VL_rise  ~ VL - VL_waning),
           VL = case_when(VL < 0 ~ 0, TRUE ~ VL),
           Infectiousness = case_when(VL<4 ~ 0, 
...
E_to_I_r <- function(df,parms,testing_parameters){
  
  df %>%
    subset(Days==Inc.pd) %>%
    dplyr::select(ID,VL,arrival,departure,V_doses,V_type,V_refused,V1_t,V2_t,VE_i,VE_s,VE_p) %>%
    mutate(asympt = rbinom(length(ID),1,(1-(1-parms[["alpha.r"]])*(1-VE_p))),
           Inf.pd=parms[["gamma"]],
           Inf.days=0,
           ID.pd=case_when(
             asympt==1 ~ round(runif(length(ID), 1, testing_parameters[["test_freq_r"]])),
             asympt==0 ~ pmin(round(runif(length(ID), 1, testing_parameters[["test_freq_r"]])), parms[["id.I"]])),
           ID.days = 0, 
...
E_to_I_hcw <- function(df,parms,testing_parameters){
  
  test = testing_parameters[["testing"]]
  
  df %>%
    subset(Days==Inc.pd) %>%
    dplyr::select(ID,VL,V_doses,V_type,V_refused,V1_t,V2_t,VE_i,VE_s,VE_p) %>%
    mutate(asympt = rbinom(length(ID),1,(1-(1-parms[["alpha.hcw"]])*(1-VE_p))),
           Inf.pd=parms[["gamma"]],
           Inf.days=0,
           ID.pd=case_when(
             asympt==1 & (V_doses!=2 | test == "outbreak") ~ round(runif(length(ID), 1, testing_parameters[["test_freq_hcw"]])), # + parms[["test_delay_hcw"]]
             asympt==0 & (V_doses!=2 | test == "outbreak") ~ pmin(round(runif(length(ID), 1, testing_parameters[["test_freq_hcw"]])), parms[["id.I"]]), # + parms[["test_delay_hcw"]]
             asympt==1 & V_doses==2 & test == "non-outbreak" ~ 1000,
             asympt==0 & V_doses==2 & test == "non-outbreak" ~ parms[["id.I"]] # + parms[["test_delay_hcw"]]                 
           ),
           ID.days = 0, 
           removal.pd = NA) -> infected
...
death <- function(df,mu,parms,total,t,dt){ # departure and death function
  ...
  # first dose for first vaccine
  VE_s1_1 = parms[["VE_s1_r1"]]
  VE_i1_1 = parms[["VE_i1_r1"]]
  VE_p1_1 = parms[["VE_p1_r1"]]
  
  # second dose for first vaccine
  VE_s2_1 = parms[["VE_s2_r1"]]
  VE_i2_1 = parms[["VE_i2_r1"]]
  VE_p2_1 = parms[["VE_p2_r1"]]
  
  # first dose for second vaccine
  VE_s1_2 = parms[["VE_s1_r2"]]
  VE_i1_2 = parms[["VE_i1_r2"]]
  VE_p1_2 = parms[["VE_p1_r2"]]
  
  # second dose for second
  VE_s2_2 = parms[["VE_s2_r2"]]
  VE_i2_2 = parms[["VE_i2_r2"]]
  VE_p2_2 = parms[["VE_p2_r2"]]
  ...
  leaving %>%
    mutate(VL = NA,arrival=t,departure= t + sample(x=LOS$LOS,size=nrow(discharges)+nrow(new_dead),prob=LOS$prob,replace=TRUE),  # alternate is + max_LOS
           V_doses = sample(c(2,1,0),size=nrow(discharges)+nrow(new_dead),prob=c(prob2,prob1,prob0),replace=TRUE),
           V_type = case_when(V_doses == 2 ~ as.numeric(sample(x=c(1,2),size=nrow(leaving),prob=c(parms[["vax1_prob"]],1-parms[["vax1_prob"]]),replace=TRUE)),
                              V_doses == 1 ~ as.numeric(sample(x=c(1,2),size=nrow(leaving),prob=c(parms[["vax1_prob_admit"]],1-parms[["vax1_prob_admit"]]),replace=TRUE))),
           V_refused = NA,
           V1_t = case_when(V_doses == 1 ~ as.numeric(t)),
           V2_t = case_when(V_doses == 1 ~ as.numeric(t)+1,
                            V_doses == 2 ~ runif(nrow(discharges)+nrow(new_dead),t-parms[["wane_day_r"]], t)),   
           VE_i = case_when(V_type == 1 & V_doses==1 & t>= V1_t+parms[["immune_delay"]] ~ VE_i1_1,
                            V_type == 1 & V_doses==2 & t>= V2_t+parms[["immune_delay"]] ~ VE_i2_1, 
                            V_type == 2 & V_doses==1 & t>= V1_t+parms[["immune_delay"]] ~ VE_i1_2,
                            V_type == 2 & V_doses==2 & t>= V2_t+parms[["immune_delay"]] ~ VE_i2_2, 
                            TRUE ~ VE_i),
           VE_s = case_when(V_type == 1 & V_doses==1 & t>= V1_t+parms[["immune_delay"]] ~ VE_s1_1,
                            V_type == 1 & V_doses==2 & t>= V2_t+parms[["immune_delay"]] ~ VE_s2_1, 
                            V_type == 2 & V_doses==1 & t>= V1_t+parms[["immune_delay"]] ~ VE_s1_2,
                            V_type == 2 & V_doses==2 & t>= V2_t+parms[["immune_delay"]] ~ VE_s2_2, 
                            TRUE ~ VE_s),
           VE_p = case_when(V_type == 1 & V_doses==1 & t>= V1_t+parms[["immune_delay"]] ~ VE_p1_1,
                            V_type == 1 & V_doses==2 & t>= V2_t+parms[["immune_delay"]] ~ VE_p2_1, 
                            V_type == 2 & V_doses==1 & t>= V1_t+parms[["immune_delay"]] ~ VE_p1_2,
                            V_type == 2 & V_doses==2 & t>= V2_t+parms[["immune_delay"]] ~ VE_p2_2, 
                            TRUE ~ VE_p)) %>%
    dplyr::select(ID,VL,arrival,departure,V_doses,V_type,V_refused,V1_t,V2_t,VE_i,VE_s,VE_p) -> new_entry
...
vaccinate <- function(df, parms, type, vax_count1, vax_count2, t){
  ...
  VE_s1_1 = if_else(type=="staff", parms[["VE_s1_hcw1"]], parms[["VE_s1_r1"]])
  VE_s2_1 = if_else(type=="staff", parms[["VE_s2_hcw1"]], parms[["VE_s2_r1"]])
  VE_i1_1 = if_else(type=="staff", parms[["VE_i1_hcw1"]], parms[["VE_i1_r1"]])
  VE_i2_1 = if_else(type=="staff", parms[["VE_i2_hcw1"]], parms[["VE_i2_r1"]])
  VE_p1_1 = if_else(type=="staff", parms[["VE_p1_hcw1"]], parms[["VE_p1_r1"]])
  VE_p2_1 = if_else(type=="staff", parms[["VE_p2_hcw1"]], parms[["VE_p2_r1"]])
  VE_s1_2 = if_else(type=="staff", parms[["VE_s1_hcw2"]], parms[["VE_s1_r2"]])
  VE_s2_2 = if_else(type=="staff", parms[["VE_s2_hcw2"]], parms[["VE_s2_r2"]])
  VE_i1_2 = if_else(type=="staff", parms[["VE_i1_hcw2"]], parms[["VE_i1_r2"]])
  VE_i2_2 = if_else(type=="staff", parms[["VE_i2_hcw2"]], parms[["VE_i2_r2"]])
  VE_p1_2 = if_else(type=="staff", parms[["VE_p1_hcw2"]], parms[["VE_p1_r2"]])
  VE_p2_2 = if_else(type=="staff", parms[["VE_p2_hcw2"]], parms[["VE_p2_r2"]])
  
  wane_day = if_else(type=="staff", parms[["wane_day_hcw"]], parms[["wane_day_r"]])
  wane_rate = if_else(type=="staff", parms[["wane_rate_hcw"]], parms[["wane_rate_r"]])
  ...
  if (t %in% parms[["t_first_dose"]]){
    ...
    # set their vaccination times and what type of vaccine they get
    df %>% 
      mutate(V_type = sample(x=c(1,2),size=nrow(df),prob=c(parms[["vax1_prob"]],1-parms[["vax1_prob"]]),replace=TRUE),
             V1_t = case_when(ID %in% df_vax$ID ~ t,  # & t>= parms[["t_first_dose"]] removed because only doing if t %in% t_first_dose
                              TRUE ~ V1_t),
  ...
  df %>% 
    mutate(V_doses = case_when(t >= V1_t & t<V2_t ~ 1, 
                               t >= V2_t ~ 2, 
                               TRUE ~ V_doses),
           VE_i = case_when(V_type == 1 & V_doses==1 & t>= V1_t+parms[["immune_delay"]] & t<V2_t+wane_day ~ VE_i1_1,
                            V_type == 1 & V_doses==2 & t>= V2_t+parms[["immune_delay"]] & t<V2_t+wane_day ~ VE_i2_1, 
                            V_type == 1 & V_doses==2 & t>=V2_t+wane_day ~ VE_i2_1 - wane_rate*(t-(wane_day + V2_t)), 
                            
                            V_type == 2 & V_doses==1 & t>= V1_t+parms[["immune_delay"]] & t<V2_t+wane_day ~ VE_i1_2,
                            V_type == 2 & V_doses==2 & t>= V2_t+parms[["immune_delay"]] & t<V2_t+wane_day~ VE_i2_2, 
                            V_type == 2 & V_doses==2 & t>=V2_t+wane_day ~ VE_i2_2 - wane_rate*(t-(wane_day + V2_t)), 
                            TRUE ~ VE_i),
           VE_s = case_when(V_type == 1 & V_doses==1 & t>= V1_t+parms[["immune_delay"]] & t<V2_t+wane_day ~ VE_s1_1,
                            V_type == 1 & V_doses==2 & t>= V2_t+parms[["immune_delay"]] & t<V2_t+wane_day ~ VE_s2_1, 
                            V_type == 1 & V_doses==2 & t>=V2_t+wane_day ~ VE_s2_1 - wane_rate*(t-(wane_day + V2_t)), 
                            
                            V_type == 2 & V_doses==1 & t>= V1_t+parms[["immune_delay"]] & t<V2_t+wane_day ~ VE_s1_2,
                            V_type == 2 & V_doses==2 & t>= V2_t+parms[["immune_delay"]] & t<V2_t+wane_day~ VE_s2_2, 
                            V_type == 2 & V_doses==2 & t>=V2_t+wane_day ~ VE_s2_2 - wane_rate*(t-(wane_day + V2_t)), 
                            TRUE ~ VE_s),
           VE_p = case_when(V_type == 1 & V_doses==1 & t>= V1_t+parms[["immune_delay"]] & t<V2_t+wane_day ~ VE_p1_1,
                            V_type == 1 & V_doses==2 & t>= V2_t+parms[["immune_delay"]] & t<V2_t+wane_day ~ VE_p2_1, 
                            V_type == 1 & V_doses==2 & t>=V2_t+wane_day ~ VE_p2_1 - wane_rate*(t-(wane_day + V2_t)), 
                            
                            V_type == 2 & V_doses==1 & t>= V1_t+parms[["immune_delay"]] & t<V2_t+wane_day ~ VE_p1_2,
                            V_type == 2 & V_doses==2 & t>= V2_t+parms[["immune_delay"]] & t<V2_t+wane_day ~ VE_p2_2, 
                            V_type == 2 & V_doses==2 & t>=V2_t+wane_day ~ VE_p2_2 - wane_rate*(t-(wane_day + V2_t)), 
                            TRUE ~ VE_p)) -> df
